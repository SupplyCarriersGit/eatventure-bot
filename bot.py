...
class EatventureBot:
    ...
    def _increment_scroll_cycle(self):
        """
        Increments the search cycle counter and handles the MAX_SCROLL_CYCLES reset.
        """
        old_val = self.search_attempt_counter
        self.search_attempt_counter += 1
        if self.search_attempt_counter > config.MAX_SCROLL_CYCLES:
            logger.info(f"Max search cycles ({config.MAX_SCROLL_CYCLES}) reached. Resetting to Cycle 1.")
            self.search_attempt_counter = 1
        logger.debug(f"Search cycle incremented: {old_val} -> {self.search_attempt_counter}")

    def execute_oscillating_search(self):
        """
        Implements Continuous Incremental Oscillating Search with Mirror Protocol.
        ...
        interval_pause = config.SCROLL_INTERVAL_WAIT

        logger.info("PHASE 1: Performing %s sequential scrolls UP", n_scrolls)
        for interval_index in range(1, n_scrolls + 1):
            up_res = self._scroll_and_scan_for_red_icons(
                "up",
            ...
            if up_res:
                logger.info(
                    "Interrupt detected after UP interval %s/%s: %s",
                    interval_index,
                    n_scrolls,
                    up_res.name,
                )
            ...
            probe_wait = interval_pause if interval_pause > 0 else 0.001
            wait_res = self._sleep_with_search_interrupt(probe_wait)
            if wait_res:
                logger.info(
                    "Interrupt detected during UP interval %s/%s wait: %s",
                    interval_index,
                    n_scrolls,
                    wait_res.name,
                )
        ...
        logger.info("PHASE 2: Performing %s sequential scrolls DOWN", n_scrolls)
        for interval_index in range(1, n_scrolls + 1):
            down_res = self._scroll_and_scan_for_red_icons(
                "down",
                config.SCROLL_DURATION,
                distance_ratio=1.0,
            )
            if down_res:
                logger.info(
                    "Interrupt detected after DOWN interval %s/%s: %s",
                    interval_index,
                    n_scrolls,
                    down_res.name,
                )
            ...
            if wait_res:
                logger.info(
                    "Interrupt detected during DOWN interval %s/%s wait: %s",
                    interval_index,
                    n_scrolls,
                    wait_res.name,
                )
